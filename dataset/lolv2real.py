"""
Dataset for LOL-v2-real (paired low-light images).
Structure:
    Train/Low/*.png     — low-light input
    Train/Normal/*.png  — normal-light GT
    Test/Low/*.png
    Test/Normal/*.png
"""
...
def _collect_pairs(base_dir):
    """Collect (low_path, normal_path) pairs from Low/ and Normal/ directories."""
    low_dir = os.path.join(base_dir, 'Low')
    normal_dir = os.path.join(base_dir, 'Normal')
    low_files = sorted(glob.glob(os.path.join(low_dir, '*.png')))
    pairs = []
    for lf in low_files:
        base = os.path.basename(lf)
        nf = os.path.join(normal_dir, base.replace('low', 'normal'))
...
class TestSet(Dataset):
    def __init__(self, args, ref_level='1', transform=transforms.Compose([ToTensor()]),
        ...
        # Files are matched by basename to the low input (e.g. low00690.png).
