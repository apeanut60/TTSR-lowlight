"""Vendored Retinexformer architecture (verbatim copy, one import removed).
...
Config matching this vendored copy: Options/RetinexFormer_LOL_v1.yml
         (n_feat=40, stage=1, level=2, num_blocks=[1,2,2])

Local changes: only the unused ``from pdb import set_trace as stx`` import was
removed (the ``stx()`` call inside ``Illumination_Estimator.forward`` was
already commented out upstream). Everything else is byte-identical to the
upstream file, so results remain comparable with the official implementation.
